...
function DiscordHook(hook,message,color) -- Baru
    local hooke = 'https://discord.com/api/webhooks/874685105162031134/X5nFBpVv4jouC_EzNNWMdTNVQBLDMB0HHF60QWIYaw26PyuNvgzwB_nGX7IH9GMGJL1P'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
...
RegisterServerEvent('esx_property:getItem') -- Baru Fix
		...
		TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = _U('player_cannot_hold') })
				else
...
--					wiadomosc = "Ambil Item Log" " \nSteam Name: " .. name .." \nItem: "..item.." x"..count.." \nID: "..source.. " \nSteam: "..steamid.." " -- Baru
--					DiscordHook('Mengambil', wiadomosc, 11750815)
				end
			else
...
RegisterServerEvent('esx_property:putItem')  -- Baruuuu Fix
...
ESX.RegisterServerCallback('esx_property:getPropertyInventory', function(source, cb, owner, propertyName) -- Baru Fix
	local _source = source
